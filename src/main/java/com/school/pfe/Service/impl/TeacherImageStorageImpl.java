...
@Service("TeacherImageStorageImpl")
public class TeacherImageStorageImpl implements ImageStorage {
    private final Path imageLocation;

    @Autowired
    public TeacherImageStorageImpl(FileStorageProperties fileStorageProperties) {
        this.imageLocation= Paths.get(fileStorageProperties.getUploadImgTeachersDir()).toAbsolutePath().normalize();
        try {
